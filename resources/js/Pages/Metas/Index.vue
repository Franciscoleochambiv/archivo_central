...
<template>
    ...
    <AuthenticatedLayout>
      <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <h2 class="mt-6 text-center text-3xl font-bold tracking-tight text-gray-900">Listado de Oficinas</h2>
        <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
          
          <button>
            <a :href="`/addmeta/`" x-data="{ tooltip: 'Añadir' }">
        ...
        />
          ...
          <table id="categoria" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
              <tr>
                ...
                <th scope="col" class="py-3 px-6">Seccion</th>
            ...
            <tbody>
              <tr v-for="(item, index) in movimientos.data" :key="index" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td class="text-xs py-2 px-6">{{ item.seccion }}</td>
                <td class="px-6 py-1">
                  <div class="flex justify-end gap-4">
                    <button>
                      <a :href="`/editmeta/${item.id}`" x-data="{ tooltip: 'Editar' }">
                        <div class="h-10 w-10 bg-red-100 dark:bg-red-800/50 flex items-center justify-center rounded-full">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" class="w-5 h-5 stroke-red-500">
          ...
          <div class="pagination-controls mt-4 flex justify-center space-x-2">
            ...
            <button
              ...
              :key="page"
              @click="fetchMetas(`/api/oficinast?page=${page}`)"
...
export default {
  ...
  methods: {
    async fetchMetas(url = '/api/oficinast?page=1') {   
    ...
    async borrar(id) {
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`/api/oficina/${id}`);
          this.fetchMetas();
    ...
    async exportToExcel() {
         ...
         const response = await fetch('/api/metaexcel');
      ...
      const headers = ['ID', 'Codigo', 'Descripcion'];
      ...
      XLSX.writeFile(workbook, 'metas.xlsx');
    }
  },
  mounted() {
    this.fetchMetas();
